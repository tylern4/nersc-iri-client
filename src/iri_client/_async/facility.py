...
class AsyncFacility:
    ...
    async def info(self) -> FacilityModel:
        """
        ...
        :rtype: Facility
        """
        r = await self._client.get("facility")
    ...
    async def sites(self) -> List[Site]:
        ...
        :rtype: List[Site]
        """
        r = await self._client.get("facility/sites")
        return [Site.model_validate(s) for s in r.json()]

    async def site(self, site_id: str) -> Site:
        """
        ...
        :rtype: Site
        """
        r = await self._client.get(f"facility/sites/{site_id}")
