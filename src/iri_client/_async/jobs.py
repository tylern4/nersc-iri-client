...
``AsyncJobSqueue`` and ``AsyncJobSacct``.  IRI collapses the distinction: a job
...
class AsyncJob(BaseModel):
    ...
    async def _job_status(self) -> JobStatus:
    ...
    async def _wait(self, timeout: int = sys.maxsize) -> "AsyncJob":
    ...
    async def wait(self, timeout: Optional[int] = None) -> "AsyncJob":
        """
    ...
    def __await__(self):
        """Allow ``await job`` to block until the job finishes."""
    ...
    async def cancel(self) -> None:
        ...
        await self._client.delete(f"compute/cancel/{self._compute.id}/{self.id}")