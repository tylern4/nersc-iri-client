...
class Compute(BaseModel):
    ...
    def submit(self, jobspec: JobSpec) -> Job:
        """
        ...
        return Job(id=json_response["id"], client=self._client, compute=self)
    ...
    def list(
        ...
        return [
            Job(id=job["id"], client=self._client, compute=self)
            for job in r.json()
        ]

    def run(self, jobspec: JobSpec) -> Job:
        """
        ...
        :rtype: Job
        """
        job = self.submit(jobspec)
        return job.wait()

    def job(self, jobid: str) -> Job:
        """
        Get a job by id.
        ...
        :rtype: Job
        """
        return Job(id=jobid, client=self._client, compute=self)